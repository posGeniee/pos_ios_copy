...
class PosScanner extends StatefulWidget {
  static const routeName = '/PosScanner';
  const PosScanner({Key? key}) : super(key: key);

  @override
  _PosScannerState createState() => _PosScannerState();
}
...
class _DetectionsCounterState extends State<DetectionsCounter> {
  ...
  void initState() {
    super.initState();
    _streamToken = codeStream.stream.listen((event) async {
      ...
      if (response.message!.data!.isEmpty) {
        ...
        Provider.of<PosSectionProvider>(context, listen: false)
            .addtoProductList(context, newData, showAlertMassage: true);
        if (!mounted) return;
        setState(() {
          isLoading = false;
        });
